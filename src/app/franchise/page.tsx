    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="p-6 rounded-2xl bg-white border border-slate-200 shadow-xs space-y-3">
          ...
          <p className="text-4xl font-black text-slate-900">₹{walletBalance.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</p>
          <p className="text-xs text-slate-500">Calculated from network gross revenue split across active hospital branches.</p>
        </div>
          ...
          {withdrawSuccess ? (
            ...
            <form onSubmit={handleWithdraw} className="space-y-4">
              ...
              <button
                ...
                className={`px-6 py-2.5 rounded-xl font-bold text-white text-xs shadow-md transition-colors ${walletBalance > 0 ? 'bg-sky-600 hover:bg-sky-700' : 'bg-slate-300 cursor-not-allowed'}`}
              >
