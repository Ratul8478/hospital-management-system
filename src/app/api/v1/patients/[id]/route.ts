...
export async function GET(
  ...
  try {
    ...
    if (!authResult.authenticated) {
      return apiError(authResult.error || 'Unauthorized: API Key or Doctor Token required to access patient record.', authResult.statusCode || 401);
    }
    ...
    return apiSuccess(patient, {
      status: 200,
      message: `Patient record for ${patient.name} (${patient.uhid}) retrieved successfully.`,
    });
  } catch (err: any) {
    return apiServerError('/api/v1/patients/[id] GET', err);
  }
}
